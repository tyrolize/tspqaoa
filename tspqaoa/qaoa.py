...
def get_ordering_swap_partial_mixing_circuit(
    ...
    if qsdk=="qiskit":
        return(q_sdk.get_ordering_swap_partial_mixing_circuit(G, i, j, u, v, beta, T,
                                                                    encoding=encoding,
                                                                    structure=structure))
...
def get_color_parity_ordering_swap_mixer_circuit(G, beta, T1, T2, encoding="onehot", qsdk="qiskit"):
    if qsdk=="qiskit":
        return(q_sdk.get_color_parity_ordering_swap_mixer_circuit(G, beta, T1, T2, encoding=encoding))


def get_simultaneous_ordering_swap_mixer(G, beta, T1, T2, encoding="onehot", qsdk="qiskit"):
    if qsdk=="qiskit":
        return(q_sdk.get_simultaneous_ordering_swap_mixer(G, beta, T1, T2, encoding=encoding))


def get_tsp_init_circuit(G, init_state=None, encoding="onehot", qsdk="qiskit"):
    if qsdk=="qiskit":
        return(q_sdk.get_tsp_init_circuit(G, init_state=init_state, encoding=encoding))
...
def get_tsp_qaoa_circuit(
    ...
    if qsdk=="qiskit":
        return(q_sdk.get_tsp_qaoa_circuit(
...
def get_tsp_expectation_value_method(G, pen, i_n=[], device="CPU", qsdk="qiskit"):
    if qsdk=="qiskit":
        return(q_sdk.get_tsp_expectation_value_method(G, pen, i_n=i_n, device=device))